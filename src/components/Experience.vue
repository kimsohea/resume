<template>
  <h2>경력</h2>
  <ul class="exper_list">
    <li v-for="(item, idx) in experArr" :key="`exper_${idx}`">
      <div class="tit_row">
      ...
      <div class="stack_box">
        <ul class="stack_group" :aria-label="`${item.tit} 기술 스택`">
          <li v-for="(tool, i) in getView(item.work)" :key="`tool_${idx}_${i}`" class="stack_avt" :title="tool" :style="getBg(tool)">
            <span v-if="getSvg(tool)" class="svg_ico" v-html="getSvg(tool)" />
            <span v-else class="txt_ico">
              {{ getTxt(tool) }}
            </span>
          </li>
          <li v-if="getRest(item.work) > 0" class="stack_avt more" :title="item.work.slice(maxCnt).join(', ')">+{{ getRest(item.work) }}</li>
        </ul>
      </div>
    </li>
  </ul>
...
const getView = (arr) => {
  ...
  return uniq.slice(0, maxCnt);
};

const getRest = (arr) => {
  const uniq = [...new Set(arr)];
  return Math.max(uniq.length - maxCnt, 0);
};
